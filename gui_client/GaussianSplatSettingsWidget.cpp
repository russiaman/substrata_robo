	...
	coverage_shrink_prev_mode(0)
{
	...
	connect(this->satBypassGridCheckBox,            SIGNAL(toggled(bool)),        this, SLOT(settingsChanged())); // SESSION081 DIAGNOSTIC, TEMPORARY
...
void GaussianSplatSettingsWidget::init(QSettings* settings_)
{
	...
	this->satBypassGridCheckBox->setChecked(false); // off. SESSION081 DIAGNOSTIC, TEMPORARY - not persisted, same reasoning: a session should not silently resume in a mode that fakes the whole occlusion result.
...
void GaussianSplatSettingsWidget::resetToDefaultsClicked()
{
	...
	this->satBypassGridCheckBox->setChecked(false); // off. SESSION081 DIAGNOSTIC, TEMPORARY - see load()'s comment.
